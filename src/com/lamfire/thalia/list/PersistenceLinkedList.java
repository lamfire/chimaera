package com.lamfire.thalia.list;
 ...
 */
public class PersistenceLinkedList<E> extends AbstractSequentialList<E> {
	...
	/**
	 * indicates that entry values should not be loaded during
	 * deserialization, used during defragmentation
	 */
	private boolean loadValues = true;

	/** constructor used for deserialization */
	PersistenceLinkedList(ThaliaBase db, long rootRecid, Serializer<E> valueSerializer) {
		this.db = db;
	...
	public PersistenceLinkedList(ThaliaBase db, Serializer<E> valueSerializer) throws IOException {
		this.db = db;
	...
	public Iterator<E> descendingIterator() {
		...
		LinkedIterator iter = new LinkedIterator();
		iter.prev = r.last;
		return new DescendingIterator(iter);
	}
	 ...
	 */
	public static PersistenceLinkedList deserialize(DataInput is, Serialization ser) throws IOException, ClassNotFoundException {
		...
		return new PersistenceLinkedList(ser.getDB(), rootrecid, serializer);
	}
	...
	public final class LinkedIterator implements ListIterator<E> {
		...
		public void remove() {
			...
			try {
				if (lastOper == 1) {
					// last operation was next() so remove
					// previous element
					lastOper = 0;
					...
					// last operation was prev() so remove
					// next element
					lastOper = 0;
		...
		public void set(E value) {
			...
			try {
				if (lastOper == 1) {
					// last operation was next(), so update
					// previous item
					lastOper = 0;
					...
					// last operation was prev() so update
					// next item
					lastOper = 0;
					Entry<E> n = db.fetch(next, entrySerializer);
					n.value = value;
		...
		public void add(E value) {
			...
			if (next == 0) {
				PersistenceLinkedList.this.add(value);
				expectedModCount++;
				return;
			}
			lock.writeLock().lock();
	...
	public static void defrag(long recid, ThaliaStore r1, ThaliaStore r2) throws IOException {
		try {
			...
			in.reset(data);
			PersistenceLinkedList l = (PersistenceLinkedList) r1.defaultSerializer().deserialize(in);
			l.setLoadValues(false);
	...
	public  class DescendingIterator  implements Iterator<E>{
		private LinkedIterator  it;
		
		public DescendingIterator(LinkedIterator it){
			this.it = it;
		}
