...
final class PageManager {
    ...
    long allocate(short type) throws IOException {
        ...
        if (type == Const.FREE_PAGE)
            throw new Error("allocate of free page?");

        // do we have something on the free list?
        long retval = headerBuf.fileHeaderGetFirstOf(Const.FREE_PAGE);                
        boolean isNew = false;
        
        if(type!=Const.TRANSLATION_PAGE){
        
            if (retval != 0) {
                // yes. Point to it and make the next of that page the
                // new first free page.
                headerBuf.fileHeaderSetFirstOf(Const.FREE_PAGE, getNext(retval));
            } else {
                // nope. make a new record
                retval = headerBuf.fileHeaderGetLastOf(Const.FREE_PAGE);
                if (retval == 0)
                    // very new file - allocate record #1
                    retval = 1;
                headerBuf.fileHeaderSetLastOf(Const.FREE_PAGE, retval + 1);
                isNew = true;
            }
        }else{
            //translation pages have different allocation scheme
            //and also have negative address
           retval = headerBuf.fileHeaderGetLastOf(Const.TRANSLATION_PAGE) - 1;
           isNew = true;
        }
    ...
    void free(short type, long recid) throws IOException {
        ...
        pageHdr.pageHeaderSetType(Const.FREE_PAGE);
        pageHdr.pageHeaderSetNext(headerBuf.fileHeaderGetFirstOf(Const.FREE_PAGE));
        pageHdr.pageHeaderSetPrev(0);

        headerBuf.fileHeaderSetFirstOf(Const.FREE_PAGE, recid);
     ...
     * {@link com.lamfire.thalia.PageFile#commit} as well).
     */
     ...
     * pages (this it's an implicit {@link com.lamfire.thalia.PageFile#commit} as well).
     */
