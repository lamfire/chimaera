 ...
 */
public class ThaliaStore extends ThaliaBase {

	/**
	 * Version of storage. It should be safe to open lower versions, but
	 * engine should throw exception while opening new versions (as it
	 * contains unsupported features or serialization)
	 */
	 ...
	 */
	private PhysicalEntryIdManager _physMgr;

	/**
	 * Indicated that store is opened for readonly operations If true, store
	 * will throw UnsupportedOperationException when update/insert/delete
	 * operation is called
	 */
	 ...
	 */
	private LogicalEntryIdManager _logicMgr;
	...
	public ThaliaStore(String filename, boolean readonly, boolean transactionDisabled, boolean lockingDisabled) throws IOException {
	 ...
	 *                 content-wise.
	 */
	public ThaliaStore(String filename, boolean readonly, boolean transactionDisabled, Cipher cipherIn, Cipher cipherOut, boolean useRandomAccessFile,
	...
	public void reopen() {
		try {
			...
			_pageMgr = new PageManager(_file);
			_physMgr = new PhysicalEntryIdManager(_file, _pageMgr);

			_logicMgr = new LogicalEntryIdManager(_file, _pageMgr);

			long versionNumber = getRoot(STORE_VERSION_NUMBER_ROOT);
			if (versionNumber > STORE_FORMAT_VERSION)
				throw new IOException("Unsupported version of store. Please update me. Minimal supported ver:" + STORE_FORMAT_VERSION + ", store ver:"
						+ versionNumber);
			if (!readonly)
	...
	public boolean isClosed() {
		return _pageMgr == null;
	}

	public synchronized <A> long insert(final A obj, final Serializer<A> serializer, final boolean disableCache) throws IOException {
		checkNotClosed();
		checkCanWrite();
		...
		if (bufferInUse) {
			// current reusable buffer is in use, have to fallback
			// into creating new instances
			DataInputOutput buffer2 = new DataInputOutput();
			return insert(obj, serializer, buffer2);
		}

		try {

			bufferInUse = true;
			return insert(obj, serializer, buffer);
		} finally {
			bufferInUse = false;
		}
	...
	private <A> long insert(A obj, Serializer<A> serializer, DataInputOutput buf) throws IOException {
		buf.reset();

		long physRowId;
		if (obj == PREALOCATE_OBJ) {
			// if inserted record is PREALOCATE_OBJ , it gets
			// special handling.
			// it is inserted only into _logicMgr with special value
			// to indicate null
			// this is used to preallocate recid for lazy inserts in
			// cache
			physRowId = PREALOCATE_PHYS_RECID;
		} else {
			serializer.serialize(buf, obj);
			if (buf.getPos() > EntryHeader.MAX_RECORD_SIZE) {
				throw new IllegalArgumentException("Too big record. JDBM only supports record size up to: " + EntryHeader.MAX_RECORD_SIZE
						+ " bytes. Record size was: " + buf.getPos());
			}
	...
	public synchronized <A> void update(long recid, A obj, Serializer<A> serializer) throws IOException {
		...
		if (bufferInUse) {
			// current reusable buffer is in use, have to create new
			// instances
			DataInputOutput buffer2 = new DataInputOutput();
			update(recid, obj, serializer, buffer2);
			return;
		}

		try {
			bufferInUse = true;
			update(recid, obj, serializer, buffer);
		} finally {
			bufferInUse = false;
		}
	}

	private <A> void update(long logRecid, final A obj, final Serializer<A> serializer, final DataInputOutput buf) throws IOException {
		...
		// previous record was only virtual and does not actually exist,
		// so make new insert
	...
	public synchronized <A> A fetch(final long recid, final Serializer<A> serializer) throws IOException {
		...
		if (bufferInUse) {
			...
			// instances
			DataInputOutput buffer2 = new DataInputOutput();
			return fetch(recid, serializer, buffer2);
		}
		try {
			bufferInUse = true;
			return fetch(recid, serializer, buffer);
		} finally {
			bufferInUse = false;
		}
	}
	...
	private <A> A fetch(long recid, final Serializer<A> serializer, final DataInputOutput buf) throws IOException {
		...
		if (physLocation == 0) {
			// throw new
			// IOException("Record not found, recid: "+recid);
			return null;
		}
		...
		buf.resetForReading();
		try {
			return serializer.deserialize(buf); // TODO there should
								// be write
								// limit to
								// throw
								// EOFException
	...
	public synchronized long getRoot(final byte id) {
		...
		return _pageMgr.getFileHeader().fileHeaderGetRoot(id);
	}
	...
	public synchronized void setRoot(final byte id, final long rowid) {
		...
		_pageMgr.getFileHeader().fileHeaderSetRoot(id, rowid);
	}
	...
	public void copyToZip(String zipFile) {
		try {
			...
			// copy zero pages
			{
				...
				z.write(Utils.encrypt(cipherIn, _pageMgr.getHeaderBufData()));
				z.closeEntry();
			}

			// iterate over pages and create new file for each
			for (long pageid = _pageMgr.getFirst(Const.TRANSLATION_PAGE); pageid != 0; pageid = _pageMgr.getNext(pageid)) {
			...
			for (long pageid = _pageMgr.getFirst(Const.FREELOGIDS_PAGE); pageid != 0; pageid = _pageMgr.getNext(pageid)) {
				PageIo page = _file.get(pageid);
				String file = zip2 + pageid;
				z.putNextEntry(new ZipEntry(file));
				z.write(Utils.encrypt(cipherIn, page.getData()));
			...
			for (long pageid = _pageMgr.getFirst(Const.USED_PAGE); pageid != 0; pageid = _pageMgr.getNext(pageid)) {
				PageIo page = _file.get(pageid);
				String file = zip2 + pageid;
				z.putNextEntry(new ZipEntry(file));
				z.write(Utils.encrypt(cipherIn, page.getData()));
			...
			for (long pageid = _pageMgr.getFirst(Const.FREEPHYSIDS_PAGE); pageid != 0; pageid = _pageMgr.getNext(pageid)) {
				PageIo page = _file.get(pageid);
				String file = zip2 + pageid;
				z.putNextEntry(new ZipEntry(file));
				z.write(Utils.encrypt(cipherIn, page.getData()));
			...
			for (long pageid = _pageMgr.getFirst(Const.FREEPHYSIDS_ROOT_PAGE); pageid != 0; pageid = _pageMgr.getNext(pageid)) {
				PageIo page = _file.get(pageid);
				String file = zip2 + pageid;
				z.putNextEntry(new ZipEntry(file));
				z.write(Utils.encrypt(cipherIn, page.getData()));
	...
	private long statisticsCountPages(short pageType) throws IOException {
		...
		for (long pageid = _pageMgr.getFirst(pageType); pageid != 0; pageid = _pageMgr.getNext(pageid)) {
			pageCounter++;
		}
		...
		try {
			...
			}
			{
				...
				for (long pageid = _pageMgr.getFirst(Const.TRANSLATION_PAGE); pageid != 0; pageid = _pageMgr.getNext(pageid)) {
					...
					for (short i = 0; i < LogicalEntryIdManager.ELEMS_PER_PAGE; i += 1) {
						final int pos = Const.PAGE_HEADER_SIZE + i * Const.PhysicalRowId_SIZE;
						...
						int availSize = EntryHeader.getAvailableSize(page, physOffset);
						int currentSize = EntryHeader.getCurrentSize(page, physOffset);
						_file.release(page);
	...
	public synchronized void defrag(boolean sortCollections) {
		...
		try {
			...
			ThaliaStore db2 = new ThaliaStore(filename2, false, true, cipherIn, cipherOut, false, false, false);

			// recreate logical file with original page layout
			{
				// find minimal logical pageid (logical pageids
				// are negative)
				...
				long pageCounter = 0;
				for (long pageid = db2._pageMgr.allocate(Const.TRANSLATION_PAGE); pageid >= minpageid; pageid = db2._pageMgr.allocate(Const.TRANSLATION_PAGE)) {
					pageCounter++;
			...
			// reinsert collections so physical records are located
			// near each other
			// iterate over named object recids, it is sorted with
			// TreeSet
			if (sortCollections) {
				...
				for (Long namedRecid : recids) {
					...
					if (obj instanceof LinkedList) {
						PersistenceLinkedList.defrag(namedRecid, this, db2);
			...
			for (long pageid = _pageMgr.getFirst(Const.TRANSLATION_PAGE); pageid != 0; pageid = _pageMgr.getNext(pageid)) {
				...
				for (int i = 0; i < LogicalEntryIdManager.ELEMS_PER_PAGE; i += 1) {
					final int pos = Const.PAGE_HEADER_SIZE + i * Const.PhysicalRowId_SIZE;
					...
					// read from logical location in second
					// db,
					// check if record was already inserted
					// as part of collections
					if (db2._pageMgr.getLast(Const.TRANSLATION_PAGE) <= pageid && db2._logicMgr.fetch(logicalRowId) != 0) {
						// yes, this record already
						// exists in second db
						continue;
					}
					...
					// read from physical location at this
					// db
					DataInputOutput b = new DataInputOutput();
					_physMgr.fetch(b, physRowId);
					byte[] bb = b.toByteArray();

					// force insert into other file, without
					// decompressing logical id to external
					// form
			...
			for (byte b = 0; b < Const.FILE_HEADER_NROOTS; b++) {
			...
			String[] exts = { Storage.EXTENSION_IDNEX_FILE, Storage.EXTENSION_DATA_FILE };
			for (String ext : exts) {
				String f1 = filename1 + ext;
				String f2 = filename2 + "_OLD" + ext;

				// first rename transaction log
				File f1t = new File(f1 + Storage.EXTENSION_TRANSACTION_FILE);
				File f2t = new File(f2 + Storage.EXTENSION_TRANSACTION_FILE);
				f1t.renameTo(f2t);
	...
	long countRecords() throws IOException {
		...
		long page = _pageMgr.getFirst(Const.TRANSLATION_PAGE);
		while (page != 0) {
			PageIo io = _file.get(page);
			for (int i = 0; i < LogicalEntryIdManager.ELEMS_PER_PAGE; i += 1) {
				int pos = Const.PAGE_HEADER_SIZE + i * Const.PhysicalRowId_SIZE;
				if (pos > Short.MAX_VALUE)
					throw new Error();
			...
			_file.release(io);
			page = _pageMgr.getNext(page);
		}
		return counter;
	}

	public void closeAndDeleteAllFiles() {
		try {
			checkNotClosed();
			super.close();
			_pageMgr.close();
	...
	static {
		int shift = 1;
		while ((1 << shift) < LogicalEntryIdManager.ELEMS_PER_PAGE)
			shift++;
	 ...
	 * This way resulting number is smaller and can be easier packed with
	 * LongPacker
	 */
	static long compressRecid(final long recid) {
		...
		offset = (short) (offset - Const.PAGE_HEADER_SIZE);
		if (offset % Const.PhysicalRowId_SIZE != 0)
			throw new InternalError("recid not dividable " + Const.PhysicalRowId_SIZE);
		long slot = offset / Const.PhysicalRowId_SIZE;
	...
	static long decompressRecid(final long recid) {
		...
		final short offset = (short) ((recid & COMPRESS_RECID_OFFSET_MASK) * Const.PhysicalRowId_SIZE + Const.PAGE_HEADER_SIZE);
