...
import com.lamfire.thalia.serializer.Serialization.FastArrayList;
...
public abstract class SerialClassInfo {
	...
	public long getSerialClassInfoRecid() {
		return serialClassInfoRecid;
	}

	public void setSerialClassInfoRecid(long serialClassInfoRecid) {
		this.serialClassInfoRecid = serialClassInfoRecid;
	}

	public ArrayList<ClassInfo> getRegistered() {
		return registered;
	}

	public void setRegistered(ArrayList<ClassInfo> registered) {
		this.registered = registered;
	}

	public static final Serializer<ArrayList<ClassInfo>> serializer = new Serializer<ArrayList<ClassInfo>>() {
	...
	public SerialClassInfo(ThaliaBase db, long serialClassInfoRecid, ArrayList<ClassInfo> registered) {
		this.db = db;
		this.serialClassInfoRecid = serialClassInfoRecid;
		this.registered = registered;
	}
		...
		private void initSetter() {
			...
			while (aClazz != Object.class) {
				...
				try {
					...
					// security manager may not be happy
					// about this
					if (!f.isAccessible())
	...
	public void writeObject(DataOutput out, Object obj, FastArrayList<?> objectStack) throws IOException {
		...
		if (classInfo.isExternalizable) {
			...
			try {
				out2.setSerializer(this);
				out2.setObjectStack(objectStack);
				o.writeExternal(out2);
			} finally {
				out2.setSerializer(null);
				out2.setObjectStack(null);
			}
			return;
		}
		...
		for (ObjectStreamField f : fields) {
			...
			if (fieldId == -1) {
				// field does not exists in class definition
				// stored in db,
				// propably new field was added so add field
				// descriptor
	...
	public Object readObject(DataInput in, FastArrayList<Object> objectStack) throws IOException {
		// read class header
		try {
			...
			if (classInfo.isExternalizable) {
				...
				try {
					in2.setSerializer(this);
					in2.setObjectStack(objectStack);
					oo.readExternal(in2);
				} finally {
					in2.setSerializer(null);
					in2.setObjectStack(null);
				}
	...
	/**
	 * Little trick to create new instance without using constructor. Taken
	 * from http://www.javaspecialists.eu/archive/Issue175.html
	 */
	...
	public abstract Object deserialize(DataInput in, FastArrayList<?> objectStack) throws IOException, ClassNotFoundException;

	public abstract void serialize(DataOutput out, Object fieldValue, FastArrayList<?> objectStack) throws IOException;
	//
