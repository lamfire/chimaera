...
/**
 * A Thalia wrapping and caching another Thalia.
 */
public class RefCachedThalia extends CachedThalia {

	private static final boolean debug = false;

	/**
	 * If Soft Cache is enabled, this contains softly referenced clean
	 * entries. If entry became dirty, it is moved to _hash with limited
	 * size. This map is accessed from SoftCache Disposer thread, so all
	...
	/**
	 * Construct a CacheRecordManager wrapping another DB and using a given
	 * cache policy.
	 */
	public RefCachedThalia(String filename, boolean readonly, boolean transactionDisabled, Cipher cipherIn, Cipher cipherOut, boolean useRandomAccessFile,
			boolean deleteFilesAfterClose, byte cacheType, boolean cacheAutoClearOnLowMem, boolean lockingDisabled) {
	...
	/**
	 * Runs in separate thread and cleans SoftCache. Runnable auto exists
	 * when CacheRecordManager is GCed
	 * 
	 * @author Jan Kotek
	 */
	static final class SoftRunnable implements Runnable {
		...
		private WeakReference<RefCachedThalia> db2;

		public SoftRunnable(RefCachedThalia db, ReferenceQueue<ReferenceCacheEntry> entryQueue) {
			this.db2 = new WeakReference<RefCachedThalia>(db);
		...
		public void run() {
			while (true)
				try {
					...
					// limit 10000 ms is to keep
					// periodically checking if db was GCed
					ReferenceCacheEntry e = (ReferenceCacheEntry) entryQueue.remove(10000);

					// check if db was GCed, cancel in that
					// case
					RefCachedThalia db = db2.get();
					if (db == null)
						return;
					...
					// otherwise SoftCacheEntries would not
					// be disposed
					e.printStackTrace();
				}
		}
