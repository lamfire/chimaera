 ...
 */
abstract class CachedThalia extends ThaliaStore {
	...
	static final class DirtyCacheEntry<A> {
		long _recid; // TODO recid is already part of _hashDirties, so
				// this field could be removed to save memory
		Object _obj;
		Serializer<A> _serializer;
	}
	...
	private Serializer<Object> cachedDefaultSerializer = null;

	/**
	 * Construct a CacheRecordManager wrapping another DB and using a given
	 * cache policy.
	 */
	public CachedThalia(String filename, boolean readonly, boolean transactionDisabled, Cipher cipherIn, Cipher cipherOut, boolean useRandomAccessFile,
	...
	boolean needsAutoCommit() {
		return super.needsAutoCommit() || (transactionsDisabled && !commitInProgress && _hashDirties.size() > NUM_OF_DIRTY_RECORDS_BEFORE_AUTOCOMIT);
	}
	...
	public synchronized <A> long insert(final A obj, final Serializer<A> serializer, final boolean disableCache) throws IOException {
		...
		final DirtyCacheEntry<A> e = new DirtyCacheEntry<A>();
		e._recid = recid;
		e._obj = obj;
		...
		try {
			synchronized (_hashDirties) {
				...
				while (!_hashDirties.isEmpty()) {
					// make defensive copy of values as
					// _db.update() may trigger changes in
					// db
					...
					for (int i = 0; i < vals.length; i++) {
						final DirtyCacheEntry<Object> entry = ((DirtyCacheEntry<Object>) vals[i]);
						vals[i] = null;
					...
					// update may have triggered more
					// records to be added into dirties, so
