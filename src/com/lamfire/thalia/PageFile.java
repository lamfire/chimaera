...
import com.lamfire.thalia.storage.StorageFactory;
...
public final class PageFile {
     ...
     */
    public static final byte[] CLEAN_DATA = new byte[Storage.PAGE_SIZE];
    ...
    PageFile(String fileName, boolean readonly, boolean transactionsDisabled, Cipher cipherIn, Cipher cipherOut, boolean useRandomAccessFile, boolean lockingDisabled) throws IOException {
        ...
        if(fileName == null){
            this.storage = StorageFactory.makeStorageMemory(transactionsDisabled);
        }else if(ThaliaBuilder.isZipFileLocation(fileName)!=null)
                this.storage = StorageFactory.makeStorageZip(ThaliaBuilder.isZipFileLocation(fileName));
//        }else if (fileName.contains("!/"))
//            this.storage = new StorageZip(fileName);
        else if(useRandomAccessFile)
            this.storage = StorageFactory.makeStorageDisk(fileName,readonly,lockingDisabled);
        else
            this.storage = StorageFactory.makeStorageDiskMapped(fileName,readonly,transactionsDisabled,lockingDisabled);
