...
/**
 * This class manages the transaction log that belongs to every {@link com.lamfire.thalia.PageFile}
...
final class PageTransactionManager {
	...
	/**
	 * In-core copy of transactions. We could read everything back from the
	 * log file, but the PageFile needs to keep the dirty pages in core
	 * anyway, so we might as well point to them and spare us a lot of
	 * hassle.
	 */
	 ...
	 * 
	 * @param owner
	 *                the PageFile instance that owns this transaction mgr.
	 * @param storage
	 * @param cipherIn
	 * @param cipherOut
	 */
	 ...
	 * consistent and guaranteed to be the only file needed for backup
	 * purposes.
	 */
	...
	private void synchronizeLogFromMemory() throws IOException {
		...
		if (txn != null) {
			// Add each page to the pageList, replacing the old copy
			// of this
			// page if necessary, thus avoiding writing the same
			// page twice
	...
	private void open() throws IOException {
		...
		oos.writeShort(Const.LOGFILE_HEADER);
		oos.flush();
		curTxn = -1;
	}
	...
	private void recover() throws IOException {
		...
		while (true) {
			...
			try {
				...
				// corrupted logfile, ignore rest of
				// transactions
				break;
			}
	...
	/**
	 * Force closing the file without synchronizing pending transaction
	...
	/**
	 * INNER CLASS. Comparator class for use by the tree set used to store
	 * the pages to write for this transaction. The PageIo objects are
	 * ordered by their page ids.
	 */
