...
import static com.lamfire.thalia.Const.*;
...
final class PageIo {
	 ...
	 * takes a snapshot so that the data may be modified in new
	 * transactions.
	 */
	...
	public long readSixByteLong(int pos) {
		long ret = ((long) (data.get(pos + 0) & 0x7f) << 40) | ((long) (data.get(pos + 1) & 0xff) << 32) | ((long) (data.get(pos + 2) & 0xff) << 24)
				| ((long) (data.get(pos + 3) & 0xff) << 16) | ((long) (data.get(pos + 4) & 0xff) << 8) | ((long) (data.get(pos + 5) & 0xff) << 0);
	...
	public void fileHeaderCheckHead(boolean isNew) {
		if (isNew)
			writeShort(FILE_HEADER_O_MAGIC, Const.FILE_HEADER);
		else {
	...
	private short fileHeaderOffsetOfFirst(int list) {
		return (short) (FILE_HEADER_O_LISTS + (2 * Const.SZ_LONG * list));
	}
	...
	private short fileHeaderOffsetOfLast(int list) {
		return (short) (fileHeaderOffsetOfFirst(list) + Const.SZ_LONG);
	}
	 ...
	 * stored in a special file, but as a large amount of space in the page
	 * header is wasted anyway, it's more useful to store it where it
	 * belongs.
	 * 
	 */
	long fileHeaderGetRoot(final int root) {
		final short offset = (short) (FILE_HEADER_O_ROOTS + (root * Const.SZ_LONG));
	...
	boolean pageHeaderMagicOk() {
		...
		return magic >= Const.PAGE_MAGIC && magic <= (Const.PAGE_MAGIC + Const.FREEPHYSIDS_ROOT_PAGE);
	}
	...
	void pageHeaderSetType(short type) {
		writeShort(PAGE_HEADER_O_MAGIC, (short) (Const.PAGE_MAGIC + type));
	}
