...
public class HTree<K, V> extends AbstractMap<K, V> implements ConcurrentMap<K, V> {
	...
	public final Serializer<Object> SERIALIZER = new Serializer<Object>() {
		...
		public Object deserialize(DataInput ds2) throws IOException {
			...
			try {
				...
				if (i == SerializationHeader.HTREE_BUCKET) { // is
										// HashBucket?
	...
	/**
	 * counts structural changes in tree at runtume. Is here to support
	 * fail-fast behaviour.
	 */
	int modCount;
	...
	/**
	 * cache writing buffer, so it does not have to be allocated on each
	 * write
	 */
	 ...
	 */
	public HTree(ThaliaBase db, Serializer<K> keySerializer, Serializer<V> valueSerializer, boolean hasValues) throws IOException {
	 ...
	 */
	public HTree(ThaliaBase db, long rootRecid, Serializer<K> keySerializer, Serializer<V> valueSerializer, boolean hasValues) throws IOException {
		this.db = db;
	...
	public void setPersistenceContext(ThaliaBase db) {
		this.db = db;
	}
	...
	public V put(K key, V value) {
		...
		try {
			...
			if (oldVal == null) {
				...
				for (EntryListener<K, V> r : recordListeners)
					r.inserted(key, value);
			} else {

				// notify listeners
				for (EntryListener<K, V> r : recordListeners)
					r.updated(key, oldVal, value);
			}
	...
	public V remove(Object key) {
		...
		try {
			...
			if (val != null) {
				...
				for (EntryListener r : recordListeners)
					r.removed(key, val);
			}
	...
	private Set<Entry<K, V>> _entrySet = new AbstractSet<Entry<K, V>>() {
		...
		public boolean remove(Object o) {
			...
			if (o instanceof Entry) {
				...
				lock.writeLock().lock();
				try {
					// get old value, must be same as item
					// in entry
					V v = get(e.getKey());
	...
	public static HTree deserialize(DataInput is, Serialization ser) throws IOException, ClassNotFoundException {
		...
		return new HTree(ser.getDB(), rootRecid, keySerializer, valueSerializer, hasValues);
	}
	...
	public static void defrag(Long recid, ThaliaStore r1, ThaliaStore r2) throws IOException {
		// TODO should modCount be increased after defrag, revert or
		// commit?
		try {
