...
import com.lamfire.thalia.ThaliaStore;
import com.lamfire.thalia.Utils;
import com.lamfire.thalia.serializer.SerialUtils;
import com.lamfire.thalia.serializer.Serialization;
import com.lamfire.thalia.serializer.SerializationHeader;
...
public final class BTreeNode<K, V> implements Serializer<BTreeNode<K, V>> {
	...
	/**
	 * Return the B+Tree that is the owner of this {@link com.lamfire.thalia.tree.BTreeNode}.
	 */
	...
	BTreeNode(BTree<K, V> btree, K key, V value) throws IOException {
		...
		_keys[BTree.DEFAULT_SIZE - 1] = null; // I am the root BTreeNode
							// for now

		_values = new Object[BTree.DEFAULT_SIZE];
		_values[BTree.DEFAULT_SIZE - 2] = value;
		_values[BTree.DEFAULT_SIZE - 1] = null; // I am the root
							// BTreeNode for now
	...
	V findValue(int height, K key) throws IOException {
		...
		if (height == 0) {
			...
			K key2 = _keys[index];
			// // get returns the matching key or the next ordered
			// key, so we must
	...
	void delete() throws IOException {
		if (_isLeaf) {
			if (_next != 0) {
				...
				if (nextNode._previous == _recid) { // this
									// consistency
									// check
									// can
									// be
			...
			if (_previous != 0) {
				...
				if (previousNode._next != _recid) { // this
									// consistency
									// check
									// can
									// be
	 ...
	 *                Insert value
	 * @param replace
	 *                Set to true to replace the existing value, if one
	 *                exists.
	 * @return Insertion result containing existing value OR a BTreeNode if
	 *         the key was inserted and provoked a BTreeNode overflow.
	 */
	InsertResult<K, V> insert(int height, K key, final V value, final boolean replace) throws IOException {
		...
		if (height == 0) {
			...
			// reuse InsertResult instance to avoid GC trashing on
			// massive inserts
			...
			overflow = -1;
			if (DEBUG) {
				System.out.println("BTreeNode.insert() Insert on leaf node key=" + key + " value=" + value + " index=" + index);
			}
			...
			// non-leaf BTreeNode
			BTreeNode<K, V> child = loadNode(_children[index], key, value);
		...
		// if we get here, we need to insert a new entry on the
		// BTreeNode before _children[ index ]
		if (!isFull()) {
			if (height == 0) {
		...
		if (index < half) {
			...
			// move lower-half of entries to overflow node, new
			// entry stays on this node
			if (DEBUG) {
		...
		if (_isLeaf) {
			...
			if (_previous != 0) {
				BTreeNode<K, V> previous = loadNode(_previous, key, value);
	...
	/**
	 * Find the first children node with a key equal or greater than the
	 * given key.
	 * 
	...
	private BTreeNode<K, V> loadNode(long recid, K k, V v) throws IOException {
		BTreeNode<K, V> child = null;
		try {
			child = loadNode(recid);
		} catch (Throwable t) {
			child = new BTreeNode<K, V>(_btree, k, v);
		}
		child._recid = recid;
		child._btree = _btree;
		return child;
	...
	/**
	 * Dump the structure of the tree on the screen. This is used for
	...
	public void serialize(DataOutput oos, BTreeNode<K, V> obj) throws IOException {
		...
		// note: It is assumed that BTreeNode instance doing the
		// serialization is the parent
		...
		if (_btree.hasValues()) {
			...
			for (int i = node._first; i < BTree.DEFAULT_SIZE; i++) {
				...
				if (header == BTreeLazyRecord.NULL) {
					...
					node._values[i] = SerialUtils.fastDeser(ois, serializer, header);
				}
			}
		} else {
			// create fake values
		...
		for (int i = node._first; i < BTree.DEFAULT_SIZE; i++) {
			if (node._values[i] instanceof BTreeLazyRecord) {
				...
				if (output.getPos() > BTreeLazyRecord.MAX_INTREE_RECORD_SIZE) {
					...
					long recid = _btree._db.insert(output.toByteArray(), SerialUtils.FAKE_SERIALIZER, true);
					oos.write(BTreeLazyRecord.LAZY_RECORD);
	...
	private void writeKeys(DataOutput oos, K[] keys, final int firstUse) throws IOException {
		 ...
		 */
		if ((_btree._comparator == Utils.getDefaultComparator() || _btree._comparator == null)
			...
			for (int i = firstUse; i < BTree.DEFAULT_SIZE; i++) {
				...
				// special case to exclude Long.MIN_VALUE from
				// conversion, causes problems to LongPacker
			...
			if (allLong) {
				// check that diff between MIN and MAX fits into
				// PACKED_LONG
				...
				// now convert to Double to prevent overflow
				// errors
				double max2 = max;
				double min2 = min;
				double maxDiff = Long.MAX_VALUE;
				if (max2 - min2 > maxDiff / 2) // divide by two
								// just to by
								// sure
					allLong = false;
	...
	public void defrag(ThaliaStore r1, ThaliaStore r2) throws IOException {
		if (_children != null)
	...
	static final class Browser<K, V> implements BTree.BTreeTupleBrowser<K, V> {
		...
		public boolean getNext(BTree.BTreeTuple<K, V> tuple) throws IOException {
			...
			if (_node == null) {
				// last record in iterator was deleted, so
				// iterator is at end of node
				return false;
			}
		...
		public boolean getPrevious(BTree.BTreeTuple<K, V> tuple) throws IOException {
			...
			if (_node == null) {
				// deleted last record, but this situation is
				// only supportedd on getNext
				throw new InternalError();
			}
		...
		public void remove(K key) throws IOException {
			...
			// An entry was removed and this may trigger tree
			// rebalance,
			// This would change current node layout, so find our
			// position again
			BTree.BTreeTupleBrowser b = _node._btree.browse(key, true);
			// browser is positioned just before value which was
			// currently deleted, so find if we have new value
	...
	/**
	 * Used for debugging and testing only. Recursively obtains the recids
	 * of all child BTreeNodes and adds them to the 'out' list.
	 * 
	 * @param out
	 * @param height
	...
	/**
	 * This method is used for delta compression for keys. Writes the
	 * contents of buf to the DataOutput out, with special encoding if there
	 * are common leading bytes in the previous group stored by this
	 * compressor.
	 * 
	 * @author Kevin Day
	 */
	static void leadingValuePackWrite(DataOutput out, byte[] buf, byte[] previous, int ignoreLeadingCount) throws IOException {
		...
		LongPacker.packInt(out, buf.length + 1);// store as +1, 0
							// indicates null
