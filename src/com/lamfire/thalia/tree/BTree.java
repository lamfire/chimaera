...
import com.lamfire.thalia.EntryListener;
...
import com.lamfire.thalia.tree.BTreeNode.InsertResult;
import com.lamfire.thalia.tree.BTreeNode.RemoveResult;
...
public class BTree<K, V> {
	 ...
	 */
	public static final int DEFAULT_SIZE = 32; // TODO test optimal size, it
							// has serious impact on
							// sequencial write and
							// read

	/**
	 * Record manager used to persist changes in BTreeNodes
	 */
	protected transient ThaliaBase _db;
	...
	/**
	 * indicates if values should be loaded during deserialization, set to
	 * false during defragmentation
	 */
	...
	/**
	 * cached instance of an insert result, so we do not have to allocate
	 * new object on each insert
	 */
	protected BTreeNode.InsertResult<K, V> insertResultReuse; // TODO
									// investigate
									// performance
									// impact
									// of
	 ...
	 */
	private transient BTreeNode<K, V> _nodeSerializer = new BTreeNode<K, V>();
	{
	 ...
	 */
	protected EntryListener[] recordListeners = new EntryListener[0];
	...
	public static <K extends Comparable, V> BTree<K, V> createInstance(ThaliaBase db) throws IOException {
	 ...
	 * Create a new persistent BTree
	 */
	public static <K, V> BTree<K, V> createInstance(ThaliaBase db, Comparator<K> comparator, Serializer<K> keySerializer, Serializer<V> valueSerializer, boolean hasValues)
			throws IOException {
		BTree<K, V> btree;
	 ...
	 */
	@SuppressWarnings("unchecked")
	public static <K, V> BTree<K, V> load(ThaliaBase db, long recid) throws IOException {
	...
	/**
	 * Get the {@link java.util.concurrent.locks.ReadWriteLock} associated with this BTree. This should
	...
	public V insert(final K key, final V value, final boolean replace) throws IOException {
		...
		try {
			...
			if (rootNode == null) {
				// BTree is currently empty, create a new root
				// BTreeNode
				if (DEBUG) {
				...
				modCount++;
				// notifi listeners
				for (EntryListener<K, V> l : recordListeners) {
				...
				if (insert._overflow != null) {
					// current root node overflowed, we
					// replace with a new root node
					if (DEBUG) {
				...
				for (EntryListener<K, V> l : recordListeners) {
					if (insert._existing == null)
						...
						l.updated(key, insert._existing, value);
				}
	...
	/**
	 * Find the value associated with the given key, or the entry
	 * immediately following this key in the ordered BTree.
	 * 
	 * @param key
	 *                Lookup key.
	 * @return Value associated with the key, or a greater entry, or null if
	 *         no greater entry was found.
	 */
	 ...
	 * <p>
	 * <b> WARNING: �If you make structural modifications to the BTree
	...
	public static BTree readExternal(DataInput in, Serialization ser) throws IOException, ClassNotFoundException {
	 ...
	 */
	public static void defrag(long recid, ThaliaStore r1, ThaliaStore r2) throws IOException {
		try {
	 ...
	 * @param listener
	 */
	public void addRecordListener(EntryListener<K, V> listener) {
	 ...
	 * @param listener
	 */
	public void removeRecordListener(EntryListener<K, V> listener) {
		List l = Arrays.asList(recordListeners);
		l.remove(listener);
		recordListeners = (EntryListener[]) l.toArray(new EntryListener[1]);
	}

	public ThaliaBase getRecordManager() {
		return _db;
	}
	 ...
	 * 
	 */
	public static interface BTreeTupleBrowser<K, V> {
		 ...
		 * @return True if values have been copied in tuple, or false if
