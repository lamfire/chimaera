...
public class BTreeMap<K, V> extends AbstractMap<K, V> implements ConcurrentNavigableMap<K, V> {
	...
	private final Set<Entry<K, V>> _entrySet = new AbstractSet<Entry<K, V>>() {
		...
		public Iterator<Entry<K, V>> iterator() {
			try {
				final BTree.BTreeTupleBrowser<K, V> br = fromKey == null ? tree.browse() : tree.browse(fromKey, fromInclusive);
	...
	public boolean inBounds(K e) {
		...
		Comparator<K> comp = comparator();
		if (comp == null)
			comp = Utils.getDefaultComparator();
	...
	public K lastKey() {
		...
		try {
			...
			if (!toInclusive && toKey != null) {
				...
				Comparator<K> c = comparator();
				if (c == null)
					c = Utils.getDefaultComparator();
	...
	public int size() {
		if (fromKey == null && toKey == null)
			return (int) tree._entries; // use fast counter on tree
							// if Map has no bounds
		else {
