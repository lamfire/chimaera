...
public final class HTreeDirectory<K, V> {
	 ...
	 * <p/>
	 * !!!! do not change this, it affects storage format, there are also
	 * magic numbers which relies on 255 !!!
	 */
	 ...
	 * <p/>
	 * (4 * 8 bits = 32 bits, which is the size of an "int", and as you
	 * know, hashcodes in Java are "ints")
	 */
	static final int MAX_DEPTH = 3; // 4 levels

	/**
	 * Record ids of children nodes. It is saved in matrix to save memory,
	 * some subarrays may be null.
	 */
	...
	/**
	 * Returns whether or not this directory is empty. A directory is empty
	 * when it no longer contains buckets or sub-directories.
	 */
	boolean isEmpty() {
	...
	V get(K key) throws IOException {
		...
		Object node = null;
		try{
			node = tree.db.fetch(child_recid, tree.SERIALIZER);
		}catch(Throwable t){
	...
	Object put(final Object key, final Object value) throws IOException {
		...
		if (child_recid == 0) {
			...
			HTreeBucket bucket = new HTreeBucket(tree, (byte) (depth + 1));
			...
			tree.db.update(recid, this, tree.SERIALIZER);
		...
		if(node == null){
			...
			tree.db.update(child_recid, this, tree.SERIALIZER);
		...
		if (node instanceof HTreeDirectory) {
			...
			if (bucket.hasRoom()) {
				...
				if (depth == MAX_DEPTH) {
					throw new RuntimeException("Cannot create deeper directory. " + "Depth=" + depth);
				}
				HTreeDirectory dir = new HTreeDirectory(tree, (byte) (depth + 1));
	...
	/**
	 * Calculates the hashmask of this directory. The hashmask is the bit
	 * mask applied to a hashcode to retain only bits that are relevant to
	...
	public void defrag(ThaliaStore r1, ThaliaStore r2) throws IOException, ClassNotFoundException {
		for (long[] sub : children) {
			if (sub == null)
				continue;
			for (long child : sub) {
				if (child == 0)
	...
	class HDIterator<A> implements Iterator<A> {
		...
		/**
		 * True if we're iterating on keys, False if enumerating on
		 * values.
		 */
		 ...
		 * 
		 * @param iterateKeys
		 *                True if iteration supplies keys, False if
		 *                iterateKeys supplies values.
		 */
		 ...
		 * <code>hasMoreElements</code>
		 * <p/>
		 * Actually, this code prepares an Enumeration on the next
		 * Bucket to enumerate. If no following bucket is found, the
			...
			do {
				...
				if (_child >= MAX_CHILDREN) {
					...
					if (_dirStack.isEmpty()) {
						// no more directory in the
						// stack, we're finished
						return;
					}
		...
		public void remove() {
			...
			// TODO current delete behaviour may change node layout.
			// INVESTIGATE if this can happen!
			tree.remove(last);
			last = null;
			expectedModCount++;
		}
