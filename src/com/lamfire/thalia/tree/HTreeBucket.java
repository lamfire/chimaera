...
final class HTreeBucket<K, V> {
            ...
            for (byte i = 0; i < size; i++) {
                ...
                if (value == null) {
                    ...
                    if (out3.getPos() > BTreeLazyRecord.MAX_INTREE_RECORD_SIZE) {
                        ...
                        long recid = tree.getRecordManager().insert(out3.toByteArray(), SerialUtils.FAKE_SERIALIZER,true);
                        out.write(BTreeLazyRecord.LAZY_RECORD);
    ...
    public void readExternal(DataInputOutput in) throws IOException, ClassNotFoundException {
        ...
        for (byte i = 0; i < size; i++) {
            ...
            K key = (K) SerialUtils.fastDeser(in, keySerializer, expectedSize);
        ...
        if(tree.hasValues()){
            ...
            for (byte i = 0; i < size; i++) {
                ...
                if (header == BTreeLazyRecord.NULL) {
                    ...
                    _keysAndValues[i + OVERFLOW_SIZE] = SerialUtils.fastDeser(in, valSerializer, header);
                }
            }
        }else{
            for (byte i = 0; i < size; i++) {
