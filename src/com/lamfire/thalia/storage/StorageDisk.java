...
import static com.lamfire.thalia.storage.StorageDiskMapped.*;
...
public class StorageDisk implements Storage {
	...
	public StorageDisk(String fileName, boolean readonly, boolean lockingDisabled) throws IOException {
		...
		// lock it
		try {
			if (!readonly && !this.lockingDisabled)
	...
	public void write(long pageNumber, ByteBuffer data) throws IOException {
		...
		// if (lastPageNumber + 1 != pageNumber) //TODO cache position
		// again, so seek is not necessary
	...
	public DataOutputStream openTransactionLog() throws IOException {
		String logName = fileName + EXTENSION_TRANSACTION_FILE;
		final FileOutputStream fileOut = new FileOutputStream(logName);
		return new DataOutputStream(new BufferedOutputStream(fileOut)) {

			// default implementation of flush on FileOutputStream
			// does nothing,
			// so we use little workaround to make sure that data
			// were really flushed
	...
	public DataInputStream readTransactionLog() {
		...
		File logFile = new File(fileName + EXTENSION_TRANSACTION_FILE);
		if (!logFile.exists())
			return null;
		if (logFile.length() == 0) {
			logFile.delete();
		...
		try {
			...
			// file should exists, we check for its presents just a
			// miliseconds yearlier, anyway move on
			return null;
		}
	...
	public static String makeFileName(String fileName, long pageNumber, int fileNumber) {
